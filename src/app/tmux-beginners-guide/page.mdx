...
<BlogPostHeader title='TMUX - Beginner&apos;s Guide 🚀' date='May 19, 2024' image={HeaderImage} />
<br />
...
It will open up a **sessions list view** that will list out all the running tmux sessions. You can navigate through the list using the arrow keys and then press `Enter` to switch to that session.
...
You can use the **sessions list view** inside tmux to kill sessions by navigating to the session with the arrow keys and pressing the `x` key. It will ask you to confirm the action, just press `y` to confirm and that particular session will be killed.
